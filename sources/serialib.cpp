...
*/
// Class constructor
serialib::serialib(int _number, unsigned int _bauds)
{
    /* Mux de la liaison série */
    serialMux(_number);

    /* Ouverture de la liaison série avec le bon bauds */
    char mot[11];
    sprintf(mot,"/dev/ttyO%d",_number);
    if(this->Open(mot,_bauds) != 1) exit(EXIT_FAILURE);
    printf("port ouvert");
}
...
/*!
 \brief revois si il y a a un octet dans le buffer
 \param pByte : data read on the serial device
 \return 1 octet lut
 \return 0 aucun octet lut
 \return -1 erreur lors de la lecture du bit
 */

char serialib::ReadCharInstant(char *pByte){
    return read(fd,pByte,1);
}
...
int serialib::ReadStringNoTimeOut(char *String,char FinalChar,unsigned int MaxNbBytes)
{
    ...
    {                                                                   // Read a byte with the restant time
        ...
        else return ret;                                          // Error while reading : return the error number
    }
